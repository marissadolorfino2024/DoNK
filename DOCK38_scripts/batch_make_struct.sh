...
#SBATCH --error=make_struct.err
#SBATCH --output=make_struct.out

# usage: this script takes as input (arg1) a directory containing pdb files of receptors to dock to and prepares the rec.pdb and xtal-lig.pdb files for docking. it creates a subdirectory in the docking directory (arg2) that includes the original pdb, rec.pdb, and xtal-lig.pdb 

# need directory with inputs names dock_inputs

shopt -s nullglob

# check for the existence of the necessary directories

# need to update below to match with these dir names
input_dir=$1
docking_dir=$2
